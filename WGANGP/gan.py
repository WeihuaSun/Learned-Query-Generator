...
class Discriminator(nn.Module):
    def __init__(self,hidden_size,num_layers,embedding_size,embedding_dim):
        ...
        self.embedding = nn.Embedding(embedding_size+1,embedding_dim,0)
...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    x_norm = torch.norm(interpolates_a.float()-interpolates_b.float(),dim=1).unsqueeze(dim=-1)
    fake = Variable(Tensor(real_samples.shape[0], 1).fill_(1.0), requires_grad=False)
    gradient_penalty = torch.mean((d_abs/x_norm-fake)**2)
    #d_mean = torch.mean(d_interpolates)
    #d_mean.backward()
    
    #fake = Variable(Tensor(real_samples.shape[0], 1).fill_(1.0), requires_grad=False)
    #fake = torch.ones(real_samples.shape[0], 1,dtype=torch.float,requires_grad=True).to(device)
    ...
    #gradients = torch.autograd.grad(
        ...
        #retain_graph=True,
        #only_inputs=True,
    #)[0]#[size = (batchsize,seqlen,tokennum)]
    #gradients = torch.pow(gradients,2)
    #gradients = torch.sum(gradients,dim=2)
    #gradients = torch.sum(gradients,dim=1)
    #gradients = torch.sqrt(gradients)
    #gradients = torch.sub(gradients,1)
    #gradients = torch.pow(gradients,2)
    #gradient_penalty = torch.sum(gradients,dim=0).mean()
